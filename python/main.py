...
def get_available_commands() -> str:
	return (
		...
		"!open [day] [hour] - Check our opening hours for a specific day and time, or just for today\n"
		"  Examples: !open Monday 14, !open Mon, !open\n"
...
@bot.command()
async def open(ctx, day: str = None, hour: Optional[int] = None):
	if not day:
		now = datetime.now()
		day = now.strftime("%A")
		if hour is None:
		...
		if hour is not None:
			if 0 <= hour < 24:
				if hours["open"] <= hour < hours["close"]:
					await ctx.send(
						f"Yes, we're open at {hour:02d}:00 on {day}. Our hours are from {hours['open']:02d}:00 to {hours['close']:02d}:00."
					)
				else:
					await ctx.send(
						f"Sorry, we're closed at {hour:02d}:00 on {day}. Our hours are from {hours['open']:02d}:00 to {hours['close']:02d}:00."
					)
			else:
